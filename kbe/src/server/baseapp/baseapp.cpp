	...
	volatileBackpressureMaxPendingBytes_(0)
{
	...
	// hook entitycall
	static EntityCall::EntityCallCallHookFunc entityCallHookFunc = std::bind(&Baseapp::createEntityCallCallEntityRemoteMethod, this,
...
bool Baseapp::installPyModules()
{
	...
	// BaseApp 同时导出完成式接口，使服务器脚本模块与 Nex 2.8 的公共 API 保持一致。
	// BaseApp exports completion APIs as well, keeping the server script module aligned with the Nex 2.8 public API.
...
void Baseapp::registerPendingLogin(Network::Channel* pChannel, KBEngine::MemoryStream& s)
{
	...
	// 登录应答下发实际绑定端口，客户端才能在 TCP 登录后切换到同一 BaseApp 的 KCP endpoint。
	// The login reply carries the bound port so the client can switch to the same BaseApp's KCP endpoint after TCP login.
