...
// System tables still contain a few direct PGresult reads, so centralize complete failure diagnostics here.
...
// kbe_entitylog startup cleanup reads kbe_serverlog, so ensure the dependency exists despite parallel system-table synchronization.
// PostgreSQL 的 IF NOT EXISTS 不能完全消除并发建表的系统目录竞争，因此同时吞掉 DDL 子事务中的重复对象和唯一键冲突。
// PostgreSQL IF NOT EXISTS does not eliminate catalog races, so absorb duplicate-object and unique-violation failures inside the DDL subtransaction.
...
// 查询邮件验证码表中的单条记录。
// Read one email-verification record while preserving NULL and binary field semantics.
...
// 判断验证码是否已过期。
// Determine expiration using the database timestamp returned with the verification record.
...
// Bind the system-table collection so schema synchronization uses PostgreSQL syntax.
...
// Synchronize kbe_entitylog, which records the baseapp currently owning each entity.
bool KBEEntityLogTablePostgresql::syncToDB(DBInterface* pdbi)
{
	// 记录实体当前所在的 baseapp。启动时会顺手清掉失效 serverGroup 留下来的在线记录，
	// Record the baseapp currently owning each entity and remove online rows left by expired server groups during startup.
	// 避免上次异常退出后，后续登录还被旧记录挡住。
	// This prevents stale rows from a prior crash from blocking later logins.
	...
	// In non-shareDB mode, match MySQL behavior by clearing historical online records during startup.
...
// 写入或更新实体在线记录。
// Insert an entity-online record for login conflict tracking.
	...
	COMPONENT_ID componentID, ENTITY_ID entityID, ENTITY_SCRIPT_UID entityType)
{
	...
	// Insert directly as MySQL does; a primary-key conflict means the entity is already online and is handled as a login conflict.
	std::string sql = fmt::format(
...
// Query an entity-online record by DBID and entity type.
...
// 删除指定实体的在线记录。
// Remove the online record for one entity.
...
// Remove every entity-online record owned by one baseapp component.
...
// 绑定系统表集合，维护服务器心跳表。
// Bind the system-table collection used to maintain server heartbeats.
...
// Synchronize kbe_serverlog, which stores server heartbeats observed by dbmgr.
bool KBEServerLogTablePostgresql::syncToDB(DBInterface* pdbi)
{
	// 这里存的是当前 dbmgr 进程组的心跳。KBE 的清理逻辑按 serverGroupID 归属实体在线记录，
	// Store the current dbmgr process-group heartbeat because KBE assigns entity-online records by serverGroupID.
	// PostgreSQL 这边也保持同一套口径。
	// PostgreSQL preserves the same ownership key and cleanup semantics.
	if (!pdbi->query(
		"DO $$ BEGIN "
		"CREATE TABLE IF NOT EXISTS kbe_serverlog ("
	...
	// An earlier PostgreSQL schema used componentID as the key, although cleanup is actually grouped by serverGroupID.
	// 这里把开发库里的旧列移走，并确保主键落在 serverGroupID 上。
	// Remove the obsolete development column and enforce serverGroupID as the primary key.
...
// Refresh the current server group's heartbeat and shareDB state atomically.
...
// Query the heartbeat record for the current server group.
...
// Query every serverGroupID still present in the heartbeat table.
...
// Find expired server groups while excluding the current process group from self-cleanup.
...
// Remove the heartbeat record for a specified server group.
...
// Read shareDB state for all groups to prevent mixed shared and non-shared deployments.
...
// Require the current process group's shareDB setting to match every live group in the database.
...
// 绑定系统表集合，维护账号信息表。
// Bind the system-table collection used to maintain account information.
...
// Synchronize kbe_accountinfos, which maps account names to entity DBIDs.
bool KBEAccountTablePostgresql::syncToDB(DBInterface* pdbi)
{
	/*
		账号表保留 KBE 的现有字段语义：name 对应 MySQL 的 accountName，
		Preserve KBE account semantics: name corresponds to MySQL accountName,
		bindata 对应 MySQL 的 bindata/blob，不能用 TEXT 顶替，否则二进制内容会被编码规则影响。
		and bindata corresponds to the binary blob; TEXT would corrupt arbitrary bytes through character encoding.
		MySQL 版本对 email 和 entityDBID 都有唯一键，
		MySQL enforces uniqueness for both email and entityDBID,
		PostgreSQL 这里也补上同等约束，避免同一邮箱或同一实体 DBID 被多条账号记录占用。
		so PostgreSQL applies equivalent constraints to prevent duplicate ownership.
		dbid=0 是“还没有绑定实体”的占位值，不能建普通唯一索引，否则多个未完成账号会互相冲突。
		DBID zero means no entity is bound yet, so a partial unique index is required to allow multiple incomplete accounts.
	*/
	if (!pdbi->query(
		"DO $$ BEGIN "
...
// 查询账号基础信息。
// Query the minimal account information required by login.
...
// 查询账号完整信息。
// Query the complete account record including binary attachment data.
...
// 创建或更新账号记录。
// Create an account or update its existing record with PostgreSQL conflict handling.
...
// 更新账号标志位和截止时间。
// Update account flags and deadline together.
...
// 更新账号关联的实体 DBID。
// Update the entity DBID associated with an account.
...
// 更新账号密码摘要。
// Update the stored account password digest.
...
// 绑定系统表集合，维护邮件验证码表。
// Bind the system-table collection used for email verification records.
...
// Synchronize kbe_email_verification, which persists email workflow state.
bool KBEEmailVerificationTablePostgresql::syncToDB(DBInterface* pdbi)
{
	// 邮件验证码表按 code 查询和清理。
	// Index verification codes because lookup and cleanup operations use the code column.
	if (!pdbi->query(
		"DO $$ BEGIN "
...
// 查询邮件验证码对应的账号信息。
// Query account information associated with an email verification code.
...
// 写入或刷新邮件验证码记录。
// Insert or refresh an email verification record.
...
// 删除指定账号和类型的邮件验证码记录。
// Delete verification records for a specific account and workflow type.
...
// 根据验证码激活账号。
// Activate an account after validating its verification code.
...
// 根据验证码绑定邮箱。
// Bind an email address after validating its verification code.
...
// 根据验证码重置密码。
// Reset an account password after validating its verification code.
