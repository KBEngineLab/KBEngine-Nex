...
DBTransaction::~DBTransaction()
{
	...
	// The destructor only performs best-effort rollback and never throws because the failed SQL has already reported the error.
	try
	{
...
bool DBTransaction::start()
{
	...
	if (pPostgresql->inTransaction())
	{
		...
		// Nested guards join the active transaction while commit and rollback remain owned by the outer guard.
		active_ = true;
		ownsTransaction_ = false;
		committed_ = false;
		return true;
