...
{
	// PostgreSQL 系统目录值可能和 UTF-8 元数据共存，因此使用公共字节安全规范化函数而不直接调用 ctype。
	// PostgreSQL catalog values may coexist with UTF-8 metadata; use the shared byte-safe normalizer instead of raw ctype calls.
	std::string value = strutil::toLower(type);
...
bool loadPostgresqlColumnInfos(DBInterface* pdbi, const std::string& tableName, PostgreSQLColumnInfos& columnInfos)
{
	...
	/*
		...
		Column synchronization needs current types, defaults, and NOT NULL state, so load the complete table metadata once.
		后面逐列判断是否真的需要 ALTER，避免“什么都没改”时也反复 SET NOT NULL。
		Each column can then decide whether ALTER is necessary instead of repeatedly issuing SET NOT NULL on unchanged schemas.
	*/
...
bool loadPostgresqlIndexNames(DBInterface* pdbi, const std::string& tableName, std::vector<std::string>& indexNames)
{
	...
	/*
		...
		This statement probes metadata during synchronization and is not application SQL.
		不写 lastquery_，否则 DBTask 慢任务日志会把这条探测 SQL 打出来，排查时容易误判。
		Do not overwrite lastquery_, otherwise slow-task diagnostics would misleadingly report the probe as business work.
	*/
	...
	pChildTable_(NULL)
{
	/*
		...
		Types such as DBID and UID are entity-definition aliases represented as their underlying integers in storage and streams.
		这里统一使用归一化后的 dataSType_，避免列同步和写入流消费各走一套判断。
		Use normalized dataSType_ consistently so schema synchronization and stream consumption cannot diverge.
	*/
	...
	const PostgreSQLColumnInfo* pColumnInfo)
{
	...
	/*
		...
		The MySQL backend continues with MODIFY COLUMN for existing fields, whereas PostgreSQL
		ADD COLUMN IF NOT EXISTS 只负责补字段，不会管类型、长度、默认值是否已经变了。
		ADD COLUMN IF NOT EXISTS only fills missing columns and does not reconcile type, length, or default changes.
		实体 def 调整 DatabaseLength 或基础类型后，如果这里不跟进，运行期读写就会和定义脱节。
		If an entity definition changes DatabaseLength or its base type, storage must follow or runtime serialization will diverge.
		因此每次同步都再执行一组幂等 ALTER：类型相同基本无成本，类型不兼容则让数据库明确报错。
		Run idempotent ALTER operations on each synchronization; matching types are cheap and incompatible changes fail explicitly.
	*/
	...
	// Backfill legacy nullable columns with the current default before enforcing NOT NULL, avoiding a manual cleanup step.
	...
	ScriptDefModule* pModule, std::vector<std::pair<std::string, std::string> >& values)
{
	...
	for (; iter != keyTypes_.end(); ++iter)
	{
		...
		/*
			...
			A fixed dictionary organizes properties but does not imply that every child is stored in the current table.
			如果子项本身需要拆表，直接让子项按自己的规则消费流并写入子表。
			When a child requires a separate table, delegate stream consumption and persistence to that child's mapping.
		*/
...
bool EntityTablePostgresql::syncToDB(DBInterface* pdbi)
{
	...
	/*
		...
		Match MySQL schema cleanup by dropping persisted columns whose ordinary properties were removed from the definition.
		id、sm_autoLoad、parentID 是框架列，不能按属性删除；FIXED_DICT 的子字段会通过
		Framework columns id, sm_autoLoad, and parentID must survive property cleanup; FIXED_DICT children are recognized through
		isSameKey() 逐列识别，组件/数组是独立子表，表级清理由 EntityTables::syncToDB() 统一处理。
		isSameKey(), while component and array tables are cleaned centrally by EntityTables::syncToDB().
	*/
	...
	/*
		...
		This registers a table-level SQL template without executing it.
		模板里保存已经按 PostgreSQL 方言处理过的表名和列名，后面如果要把整行查询、
		The template retains PostgreSQL-qualified table and column names so future whole-row reads,
		prepared statement 或批量写入收敛起来，不需要再从 EntityTableItem 重新扫字段。
		prepared statements, or batch writes can reuse them without rescanning EntityTableItem objects.
	*/
...
bool EntityTablePostgresql::syncIndexToDB(DBInterface* pdbi)
{
	...
	/*
		...
		MySQL synchronizes ordinary property indexes from indexType(), while the previous PostgreSQL path created only
		autoLoad/parentID 两个内部索引，业务字段上的 UNIQUE/INDEX 会被悄悄忽略。
		the internal autoLoad and parentID indexes and silently ignored application UNIQUE or INDEX declarations.
		这里不主动删除旧索引，先做到“定义里要求的索引一定存在”；删索引涉及线上数据和查询计划，
		Ensure every requested index exists but do not remove old indexes automatically because removal affects live data and query plans.
		应该由迁移脚本显式处理，更稳一点。
		Obsolete indexes should be handled explicitly by a reviewed migration script.
	*/
...
bool EntityTablePostgresql::queryFixedOrderItems(DBInterface* pdbi, DBID dbid, MemoryStream* s, ScriptDefModule* pModule)
{
	...
	if (pQueryStatement != NULL && !pQueryStatement->columns().empty())
	{
		...
		/*
			...
			The mapping template begins with framework columns, while entity stream output consumes only property columns.
			主表跳过 sm_autoLoad，子表还要再跳过 parentID。
			Skip sm_autoLoad for root tables and skip both sm_autoLoad and parentID for child tables.
		*/
...
DBID EntityTablePostgresql::writeTable(DBInterface* pdbi, DBID dbid, int8 shouldAutoLoad, MemoryStream* s, ScriptDefModule* pModule)
{
	...
	while (s->length() > 0)
	{
		...
		// Empty account-entity templates may contain only a trailing 0,0 sentinel, which must not be interpreted as property data.
