...
namespace KBEngine {
	namespace
	{
		...
		// 统一创建单字段索引并保证 BSON 生命周期在驱动调用完成后结束。
		// Create a single-field index consistently and keep BSON ownership bounded to the synchronous driver call.
		bool ensure_index(DBInterface* pdbi, const char* collection, const char* field, bool unique)
		{
			bson_t keys;
			bson_t options;
			bson_init(&keys);
			...
			bool result = mongoDBI(pdbi)->collectionCreateIndex(collection, &keys, &options);
			bson_destroy(&options);
			bson_destroy(&keys);
			return result;
		}

		// 游标耗尽后统一提取延迟报告的网络或服务端错误，并写回数据库接口诊断状态。
		// Extract deferred network or server errors after cursor exhaustion and persist them in the database interface diagnostics.
		bool cursor_succeeded(DBInterface* pdbi, mongoc_cursor_t* cursor, const char* operation)
		{
			bson_error_t error = {};
	...
	bool KBEEntityLogTableMongodb::syncToDB(DBInterface* pdbi)
	{
		...
		if (ret == 0)
		{
			bson_t empty;
			bson_init(&empty);
			bool cleared = mongoDBI(pdbi)->collectionRemove("kbe_entitylog", MONGOC_REMOVE_NONE, &empty, NULL);
			bson_destroy(&empty);
			if (!cleared)
				return false;
		}
		...
		bool entityKeyReady = pdbiMongodb->collectionCreateIndex("kbe_entitylog", &keys, &opts);
		bson_destroy(&opts);
		bson_destroy(&keys);
		if (!entityKeyReady)
			return false;

		if (!ensure_index(pdbi, "kbe_entitylog", "serverGroupID", false))
			return false;
		...
		if (!cids.empty())
		{
			...
			{
				if (!pdbiMongodb->collectionRemove("kbe_entitylog", MONGOC_REMOVE_NONE, &query, NULL))
				{
					bson_destroy(&query);
					return false;
				}
		...
		if (!cursor_succeeded(pdbi, guard->cursor(), "KBEEntityLogTableMongodb::syncToDB"))
		{
			bson_destroy(&query);
			return false;
		}
	...
	bool KBEEntityLogTableMongodb::queryEntity(DBInterface* pdbi, DBID dbid, EntityLog& entitylog, ENTITY_SCRIPT_UID entityType)
	{
		...
		if (!cursor_succeeded(pdbi, guard->cursor(), "KBEEntityLogTableMongodb::queryEntity"))
		{
			bson_destroy(&query);
			return false;
		}
	...
	{
		DBInterfaceMongodb* pdbiMongodb = mongoDBI(pdbi);
		if (!pdbiMongodb->createCollection("kbe_serverlog") ||
			!ensure_index(pdbi, "kbe_serverlog", "serverGroupID", true))
			return false;
	...
	std::vector<COMPONENT_ID> KBEServerLogTableMongodb::queryServers(DBInterface* pdbi)
	{
		...
		cursor_succeeded(pdbi, guard->cursor(), "KBEServerLogTableMongodb::queryServers");
	...
	std::vector<COMPONENT_ID> KBEServerLogTableMongodb::queryTimeOutServers(DBInterface* pdbi)
	{
		...
		cursor_succeeded(pdbi, guard->cursor(), "KBEServerLogTableMongodb::queryTimeOutServers");
	...
	std::map<COMPONENT_ID, bool> KBEServerLogTableMongodb::queryAllServerShareDBState(DBInterface* pdbi)
	{
		...
		cursor_succeeded(pdbi, guard->cursor(), "KBEServerLogTableMongodb::queryAllServerShareDBState");
	...
	{
		DBInterfaceMongodb* pdbiMongodb = mongoDBI(pdbi);
		if (!pdbiMongodb->createCollection("kbe_email_verification") ||
			!ensure_index(pdbi, "kbe_email_verification", "code", true))
			return false;
		...
		for (size_t i = 0; i < sizeof(rules) / sizeof(rules[0]); ++i)
		{
			...
			bool removed = pdbiMongodb->collectionRemove("kbe_email_verification", MONGOC_REMOVE_NONE, &query, NULL);
			bson_destroy(&query);
			if (!removed)
				return false;
		}
	...
	bool KBEEmailVerificationTableMongodb::resetpassword(DBInterface* pdbi, const std::string& name, const std::string& password, const std::string& code)
	{
		...
		if (!pTable->updatePassword(pdbi, qname, KBE_MD5::getDigest(password.data(), static_cast<int>(password.length()))))
			return false;
