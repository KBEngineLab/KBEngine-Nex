...
protected:
	...
	bool setReadEnabled(KBESOCKET fd, SocketState& state, bool enabled);
	void updateReadBackpressure(KBESOCKET fd, SocketState& state);

	// 检查 kevent 是否仍属于当前读生命周期，并刷新 socket 类型。
	bool refreshSocketKind(KBESOCKET fd, SocketState& state);
	bool isReadEventCurrent(KBESOCKET fd, SocketState& state);
	...
	int drainAccept(KBESOCKET fd, SocketState& state);

	// 从 TCP socket 读取一次，并把结果转成 TCP completion。
	int drainTcpRead(KBESOCKET fd, SocketState& state);

	// 从 UDP socket 读取一个 datagram，并把结果转成 UDP completion。
	int drainUdpRead(KBESOCKET fd, SocketState& state);

	// 尝试发送 TCP/UDP 队列，并在完成后触发写 completion。
	int flushPendingSends(KBESOCKET fd, SocketState& state);
