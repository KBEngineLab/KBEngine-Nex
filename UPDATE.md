...
- 修复Witness绑定的时序导致view范围被默认xml配置覆盖的bug
- 修复组件发现启动阶段瞬时超时问题，每轮最多进行 5 次探测，每次重新申请临时端口并广播，仅在单轮探测全部失败后进入外层重试
...
- [fix] 修复newassets时的一处bug

## 3.0.3

- [feat] 移除SDK远程下载能力，防止客户端通过特殊手段下载SDK

## 3.0.2

- [fix] 补充压测宿主容量饱和诊断
  - 在同一次操作系统快照内聚合控制器、服务端组件和负载进程的 CPU、工作集与进程数，新增 `process.owned-total.*` 指标，避免错误叠加各进程分位数。
  - 新增 `owned_cpu_p95_max_percent` 容量阈值；自有进程 CPU P95 超限时将报告标记为 `SLOW`，明确区分引擎长尾与单机压测宿主饱和。
  - 补充指标契约测试和中英文压测文档，固定聚合口径、默认阈值与报告诊断行为。

- [fix] 补齐性能压测脚本错误统计
  - 错误日志采集兼容 Logger 的 `S_ERR`、组件标准输出的 `[S_ERROR]` 以及带组件名前缀的严重级别格式，避免脚本异常漏过质量门禁。
  - 错误匹配继续限定在日志行首，避免将业务字段中的 `error=...` 或正文中的错误关键字误计为异常。
  - 增加分块增量读取、格式兼容和误报防护测试，确保跨采样周期不会重复统计。

- [performance] 对齐 Linux io_uring 与 Windows IOCP 完成模型
  - 统一 TCP accept、收发、取消、重投递和完成队列生命周期；空闲连接首包立即提交，发送按批次推进，并通过高低水位背压限制长业务回调期间的用户态积压。
  - TCP 接收在进入 PacketReader/Entity 回调前预挂下一次读请求；完成项按数量和时间预算公平调度，避免网络突发饿死 Timer、Tick 和其他连接。
  - io_uring UDP 使用单等待请求配合有界批量 drain，IOCP 保留多 outstanding buffer；两端统一逻辑 completion 计数、优先级调度、关闭竞态清理和可观测指标。
  - Channel 索引纳入 TCP/UDP 传输类型，修复相同地址端口下 TCP 与 KCP 路由互相遮蔽；同步完善 KCP 调度、内部连接活性判断及完成模型背压诊断。
  - 扩充 completion 公平性、TCP 发送队列、UDP 接收深度、跨 Cell 迁移与路由契约等回归测试，并优化万人压测进程拓扑和跨平台运行配置。
...
- [security] 完成服务端跨组件入口与会话路由安全收口
  ...
  - 修复回调超时未主动回收、管理器互注册连接竞态、加密握手竞态等生命周期问题。
  - 加固客户端 SDK 的 Blowfish 帧长度与解密输入校验，避免异常数据导致帧状态错乱或越界处理。
  - 覆盖客户端请求、组件注册、数据库操作、跨 Cell 迁移、计费和 Interfaces 生命周期等高风险入口，统一拒绝异常来源和失效会话。

- [performance] 优化 KCP、UDP、IOCP 调度与高并发网络背压
  - 重构 KCP ACK、数据发送、单通道 Flush 和更新队列的调度预算，提升多连接下的公平性，降低无效扫描、接收空窗和调度长尾。
  - 限制外部 KCP 字节积压和单客户端缓冲增长，合并可合并的发送段，抑制背压反馈放大，改善万级 Bots 下的连接稳定性。
  - 增加 IOCP completion 批量处理和自适应预算，避免单轮处理过量放大 Tick 长尾；同时保留 TCP 小包合并与发送队列优化。
  - 增加 KCP 重传、ACK、窗口阻塞、UDP 背压、IOCP completion 和调度延迟等指标，为网络参数调优和正式压测提供依据。

- [performance] 优化 AOI、Witness、Cell Tick 和 Space 分配
  - 对 AOI dirty 广播、观察关系、位姿同步和坐标递归扫描进行去重，减少同一关系重复入队及高密度 AOI 的更新放大。
  - 将 Witness 待处理数据按压力分层调度，优先处理有积压的 Witness，排除空视野对象，降低脏队列和位置同步长尾。
  - 优化 CoordinateSystem、Updatable、RangeTrigger 等移动热路径的数据结构和重复距离/平方根计算，减少递归扫描和非连续内存访问。
  - CellAppMgr 创建 Space 时综合已确认 Space、待确认 Space、Witness 压力和实时负载进行分配，降低新 Space 集中到单个 CellApp 的风险。
  - 增加 Cell Tick、Cell 迁移、AOI、触发器、Python 回调和消息分段的分段耗时指标，用于定位 P95/P99/P99.9 长尾。

- [performance] 建立可复现的多组件、多 Bots 标准压测体系
  - 压测工程支持通过参数传入 assets、服务端组件数量、Bots 数量、启动批次和运行时长，并以服务端 Watcher/业务就绪状态作为启动门槛。
  - 支持多个 Bots 进程聚合运行，支持 TCP/KCP 协议切换，补充 BaseApp、CellApp、Python 到底层调用和整体服务器的 P50/P95/P99/P99.9 指标。
  - 增加进程 CPU、私有内存、峰值内存、网络收发、Tick、KCP、IOCP、Watcher 和消息处理等标准采样字段，支持短时对照压测和正式报告输出。
  - 增加严格失败诊断、启动清理、日志生命周期隔离、Watcher 控制面复用及低扰动采样机制，降低压测工具本身对结果的影响。

- [fix] 改善 Bots 移动、登录和压测运行稳定性
  - 修复 KCP 背压、客户端 Tick 排队和过期移动输入导致的 Bots 长时间停顿，提升高并发场景下的移动推进公平性。
  - 修复 Bots 移动回调生命周期和 AOI 语义消息清理问题，减少异常断线、重复回调和连接清理不完整。
...
- [feat] 扩展工具、协议和跨平台构建能力
  ...
  - vcpkg baseline 统一迁移到 `kbengine1x` 工程使用的固定版本，主工程依赖清单和构建工具链改为可复现配置。
...
- [security] 收口数据库与组件生命周期数据流
  ...
  - 收口 DBTask 异步回调目标会话、回调超时回收、实体销毁和 Interfaces 生命周期，避免过期任务回调到已失效对象。
  - 组件注册和端点发现增加无效端点、重复连接和连接状态校验，降低异常注册导致 BaseApp/CellApp 断言或错误退出的风险。
  - 对跨 Cell 迁移、Ghost 更新、客户端入口和计费相关消息增加来源、目标和状态约束，避免迁移过程中的重入和越权调用。

- [security] 加固客户端 SDK 与加密协议边界
  - 统一 C++、C#、GDScript SDK 对 Blowfish 帧长度、解密输入、消息边界和异常状态的处理规则。
  - 修复加密握手与 BaseApp 连接建立期间的竞态，避免过滤器安装前后收到异常帧时造成状态错乱。
  - 保留账户、邮箱、登录名和重连名等调试追踪信息，同时对网络输入和服务端实体状态进行底层校验。

- [performance] 完善 Watcher 与全链路低扰动性能观测
  - 增加 BaseApp、CellApp、Bots 和整体服务器的 CPU、私有内存、峰值内存、网络收发、Tick、消息处理和队列指标。
  - 增加 Python 到底层调用延迟、BaseApp/CellApp 真实业务调用分位数，以及 P50、P95、P99、P99.9 统计口径。
  - Watcher 控制面采用复用连接、分目标低频采样和错峰调度，减少采样请求自身造成的 IOCP、网络和 Tick 抖动。
  - 性能探针默认关闭，只有显式开启时才采集详细诊断数据，避免生产环境产生额外 CPU、内存和日志开销。

- [performance] 优化 Cell 迁移、消息分段和 Tick 长尾
  - 拆分 Python、Cell 迁移构造、迁移回调、AOI 和消息分段耗时，能够区分业务脚本耗时与底层调度耗时。
  - 治理过期移动输入、慢 Tick 日志放大和迁移失败状态，避免异常请求长期占用调度队列。
  - 增加 Cell Tick 分段延迟、慢轮询和待处理队列诊断，为 P99/P99.9 长尾定位提供调用链证据。

- [performance] 优化 BaseApp 客户端分配与跨进程公平性
  - 按客户端压力和迁移状态均衡 BaseApp 客户端，降低新连接集中到单个 BaseApp 导致的单核长尾。
  - 增加 BaseAppMgr、CellAppMgr 的待创建、已确认和实时负载统计，创建请求发出后立即计入预留压力。
  - 限制 Witness 待处理负载和 Space 集中分配，兼顾 Space 数量、Witness 压力和实时 CPU 负载。

- [feat] 完善服务发现、端口和运行目录约定
  - 优化 Machine 端点发现超时和发现过程指标，减少服务启动阶段的无效等待。
  - 统一服务端 TCP/UDP 外部端口配置，避免 TCP 与 KCP 配置分散造成启动脚本和客户端连接不一致。

- [tool] 完善压测工程的可操作性与失败诊断
  - 支持 assets 路径、BaseApp/CellApp 数量、Bots 总数、批次、协议和运行时长参数化。
  - 通过底层就绪事件和 Watcher 聚合判断服务端启动完成，避免组件尚未全部注册时提前启动 Bots。
  - 支持本机多 Bots 进程聚合，具备连接建立、登录、角色选择/创建、移动、重连和异常清理等完整业务流程。
  - 增加压测失败原因、进程残留、服务端日志、连接流失、握手超时和未知消息 ID 的统一诊断输出。

- [tool] 优化 Bots 日志与业务运行模式
  - 增加 `dev` 启动模式：开发模式可将日志转发给 Logger 供 IDE 插件显示，非开发模式默认写入 Bots 自身日志文件。
  - 将压测日志按启动、连接、稳态、采样和清理阶段隔离，避免日志放大影响网络和 CPU 指标。
  - 支持 TCP/KCP 协议切换压测，为同一业务流程提供协议对照基线。

- [test] 建立跨平台构建与回归测试闭环
  - Windows、Linux/WSL2、macOS 统一使用 CMake Preset、vcpkg manifest 和 CTest 入口，减少平台专用工程差异。
  - 主工程和 Python runtime 使用独立 triplet 与固定 baseline，依赖恢复路径可审计、可复现。
  - 完善网络、KCP、IOCP、AOI、Witness、CellAppMgr 分配、Blowfish 和安全契约测试，并纳入 `kbe_tests` 聚合目标。
