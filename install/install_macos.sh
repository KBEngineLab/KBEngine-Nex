...
require_command() {
    command -v "$1" >/dev/null 2>&1 || {
        echo "[ERROR] 未找到 $1，请先安装 Git、CMake、Ninja 和 Xcode Command Line Tools。" >&2
        exit 1
    }
}

for tool in git cmake ninja xcrun; do require_command "$tool"; done
...
cd "$SOURCE_DIR"
cmake --fresh --preset macos-ninja
